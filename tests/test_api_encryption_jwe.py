...
from tests import get_mastercard_config_for_test, JWE_TEST_CONFIG, get_jwe_config_for_test
...
class ApiEncryptionJweTest(unittest.TestCase):
    ...
    @patch('client_encryption.api_encryption.JweEncryptionConfig')
    def test_ApiEncryption_with_config_as_file_name(self, JweEncryptionConfig):
        to_test.ApiEncryption(JWE_TEST_CONFIG, "JWE")

        assert JweEncryptionConfig.called

 
    @patch('client_encryption.api_encryption.JweEncryptionConfig')
    def test_ApiEncryption_with_config_as_dict(self, JweEncryptionConfig):
        to_test.ApiEncryption(self._json_config, "JWE")
    ...
    def test_encrypt_payload_returns_same_data_type_as_input(self):
        api_encryption = to_test.ApiEncryption(self._json_config, "JWE")
    ...
    def test_decrypt_payload_with_params_in_headers(self):
        ...
        self.assertDictEqual({"Content-Type": "application/json", 
                        ...
                        "x-cert-fingerprint": "gIEPwTqDGfzw4uwyLIKkwwS3gsw85nEXY0PP6BYMInk=",
                        "x-key-fingerprint": "dhsAPB6t46VJDlAA03iHuqXm7A4ibAdwblmUUfwDKnk=",
                        "x-oaep-digest": "SHA256"}, test_headers)
    ...
    def test_decrypt_payload_with_params_in_headers_skip_decrypt(self):
        ...
        self.assertDictEqual({"data": {"key1": "notSecret", "key2": "anotherValue"}}, json.loads(decrypted))
        self.assertEqual(5, len(test_headers.keys()))

    @patch('client_encryption.api_encryption.JweEncryptionConfig')
    def test_add_header_encryption_layer_with_config_as_file_name(self, JweEncryptionConfig):
        to_test.add_encryption_layer(MockApiClient(), JWE_TEST_CONFIG, "JWE")

        assert JweEncryptionConfig.called

    @patch('client_encryption.api_encryption.JweEncryptionConfig')
    def test_add_header_encryption_layer_with_config_as_dict(self, JweEncryptionConfig):
        to_test.add_encryption_layer(MockApiClient(), self._json_config, "JWE")
